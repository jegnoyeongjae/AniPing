...
const AdminSettingLi = ({realAdmin, index, handleClickRemoveAdmin }) => {
    ...
    const onDeleteAdmin = () => {
        if(confirm(`'${realAdmin.userId}' 사용자의 운영자 권한을 해제하시겠습니까?`)){
    ...
    return(
        <li className="grid grid-cols-12 gap-4 p-5 items-center hover:bg-slate-50/50 transition-colors text-center">
            ...
            <div className="col-span-2 font-bold text-slate-700">{realAdmin.userId}</div>
            ...
            <div className="col-span-2 text-slate-400 text-sm">{realAdmin.startDate}</div>
